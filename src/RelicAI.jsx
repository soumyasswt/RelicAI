...
// STREAMING API
// ═══════════════════════════════════════════════════════════════
async function streamOllama({ messages, system, tools = [], maxTokens = 2500, onToken, onTool, onDone, onError }) {
  try {
    const formattedMsgs = system ? [{ role: "system", content: system }, ...messages] : messages;
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 60000); // 60s timeout
    const resp = await fetch("http://localhost:11434/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "llama3", messages: formattedMsgs, stream: true }), signal: controller.signal });
...
async function callOllama(messages, system, opts = {}) {
  try {
    ...
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 60000); // 60s timeout
    const resp = await fetch("http://localhost:11434/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "llama3", messages: formattedMsgs, stream: false }), signal: controller.signal });
    clearTimeout(timeoutId);
...
const DEFAULT_SETTINGS = { chunkSize: 450, chunkOverlap: 60, topK: 6, webSearchDefault: false, streamingEnabled: true, showConfidence: true, autoExtractEntities: true, maxConvHistory: 20, systemPrompt: `You are an expert AI research assistant.\nAnswer using ONLY the provided context. Cite sources as [1],[2] etc.\nIf context is insufficient, say so clearly. Be thorough and structured.\nEnd answers with a "**Sources:**" section listing every source used.` };
const SEED = [
...
export default function App() {
  ...
  const addLog = useCallback((msg, type = "info") => { const e = { msg, type, ts: Date.now() }; logsRef.current = [...logsRef.current.slice(-80), e]; setLogs([...logsRef.current]); }, []);
  ...
  const extractEntities = useCallback(async (text, docId, title) => {
    ...
    try {
      const raw = await callOllama([{ role: "user", content: `Extract entities and relationships from this text. Return ONLY valid JSON:\n{"nodes":[{"id":"snake_case","label":"Display Name","type":"technology|concept|person|organization"}],"edges":[{"s":"id1","t":"id2","label":"verb"}]}\nKeep to 8-12 most important entities.\n\nText:\n${text.slice(0, 1800)}` }], "Return only valid JSON, no markdown fences.", { maxTokens: 700 });
  ...
  const handleQuery = useCallback(async (e) => {
    ...
    try {
      ...
      retrieved.slice(0, 3).forEach((c, i) => addLog(`   [${i + 1}] "${c.meta?.title?.slice(0, 35)}" s=${c.score.toFixed(3)}`, "dim"));
      ...
      const tools = needsWeb ? [{ type: "web_search_20250305", name: "web_search" }] : [];

      addLog("🧠 Streaming from Ollama…", "info");

      // Set up timeout for the entire operation
      timeoutId = setTimeout(() => {
        addLog("⏰ Response timeout (60s) — Ollama may be busy or offline", "error");
        setStreaming(false);
        setStreamText("Error: Response timeout. Try again or check Ollama status.");
      }, 60000);

      let answer = "";
      await streamOllama({
        messages: msgs, system: settings.systemPrompt, tools, maxTokens: 2500,
        onToken: (_, full) => {
          setStreamText(full);
          answer = full;
          if (timeoutId) clearTimeout(timeoutId); // Clear timeout on first token
        },
        onTool: (name, state) => {
          if (state === "start") {
            setToolActive("Searching: " + name);
            addLog("   🌐 Web search triggered", "info");
        ...
        onError: (err) => {
          ...
          if (err.includes('404')) {
            addLog("💡 Model not found. Run: ollama pull llama3", "info");
          } else if (err.includes('fetch')) {
            addLog("💡 Ollama not running. Start with: ollama serve", "info");
          }
          answer = `Error: ${err}`;
        },
      });
    ...
    } catch (err) { addLog(`❌ ${err.message}`, "error"); setConversation(c => [...c, { role: "assistant", content: `Error: ${err.message}`, ts: Date.now() }]); }
    setStreaming(false); setStreamText(""); setToolActive(null);
  }, [query, streaming, conversation, settings, queryHistory]);
  ...
  const doIngest = useCallback(async (text, meta) => {
    ...
    await saveKB(); extractEntities(text, r.id, meta.title); return r;
  ...
  }, [ingestText, ingestTitle, ingesting, doIngest]);

  const fetchAndIngest = useCallback(async (url, title) => {
    ilog(`Fetching: ${url.slice(0, 60)}…`, "info");
    try {
      const content = await callOllama([{ role: "user", content: `Fetch and extract the complete main content from this URL. Return the full article/paper text, all technical details and key information. 500+ words minimum.\nURL: ${url}` }], "Return only the extracted content, no preamble.", { webSearch: true, maxTokens: 2500 });
  ...
  const ingestArxiv = useCallback(async () => {
    ...
    try {
      const content = await callOllama([{ role: "user", content: `Retrieve and summarize this arXiv paper comprehensively: ${ingestUrl}\nInclude: abstract, problem, methodology, architecture, algorithms, results, benchmarks, contributions, limitations, conclusions. 800+ words.` }], "Return detailed technical paper content.", { webSearch: true, maxTokens: 2800 });
      ilog(`Fetched: ${content.split(" ").length} words`, "success");
    ...
    setIngesting(false);
  }, [ingestUrl, ingestTitle, ingesting, doIngest]);
  ...
  }, [bulkUrls, ingesting, fetchAndIngest]);

  // ── EVALUATION ────────────────────────────────────────────
  const EVAL_TESTS = [
    { q: "Explain how transformer attention mechanisms work", kw: ["attention", "query", "key", "value"], cat: "research" },
    { q: "Compare BM25 versus TF-IDF for document retrieval", kw: ["bm25", "tfidf", "saturation", "idf"], cat: "comparison" },
    { q: "What is RAG and how does it reduce hallucinations?", kw: ["retrieval", "context", "grounding"], cat: "research" },
    { q: "How does Llama 3 differ from previous open-source models?", kw: ["llama", "parameter", "benchmark"], cat: "factual" },
    { q: "Explain hybrid retrieval with RRF fusion", kw: ["hybrid", "rrf", "fusion", "vector", "keyword"], cat: "research" },
  ];
  ...
  }, [evalRunning, settings.systemPrompt]);
...
*{box-sizing:border-box;margin:0;padding:0;}body{background:${T.bg};}::-webkit-scrollbar{width:3px;height:3px;}::-webkit-scrollbar-track{background:${T.bg};}::-webkit-scrollbar-thumb{background:${T.border};border-radius:2px;}.spin{animation:spin 0.9s linear infinite;}@keyframes spin{to{transform:rotate(360deg);}}.blink{animation:blink 1s step-end infinite;}@keyframes blink{50%{opacity:0;}}.fadein{animation:fadein 0.3s ease-out;}@keyframes fadein{from{opacity:0;transform:translateY(8px);}to{opacity:1;transform:translateY(0);}}input,textarea,button{font-family:${T.mono};}textarea{resize:vertical;}`;

  if (!ready) return (
    <div style={{ background: T.bg, height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: T.mono }}>
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "'DM Sans',sans-serif", background: T.bg, height: "100vh", display: "flex", flexDirection: "column", color: T.text, overflow: "hidden" }}>
        <style>{css}</style>
